            ...
            <div class="col-md-12">
                <div class="card">
                    @if( isset($query) && !empty($query) )
                        <div class="card-header"><strong>{{__('Видео найдены по запросу')}} - {!! $query !!}</strong></div>
                    @else
                        <div class="card-header">{{__('Все видео')}}</div>
                    @endif
                    <div class="card-body">
                        @if(\count($articles))
                            <div class="row">
                                @foreach($articles as $article)
                                    <div class="col-md-2">
                                        <div class="article">
                                            <div class="card mb-4">
                                                <video class="card-img-top" style="object-fit: cover;" src="{{ env('APP_URL') . '/storage/videos/' . $article->video }}#t=0.5" alt="{{ $article->fullName }}">
                                                </video>
                                                <div class="card-body">

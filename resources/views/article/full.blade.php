                                        ...
                                        <div class="media mb-4">
                                            ...
                                            <div class="media-body">
                                                <div class="justify-content-between">
                                                    ...
                                                    <div class="last">
                                                        @if(auth()->user()->id !== $article->user()->select('id')->first()->id )
                                                            <?php
                                                                $authUser = auth()->user()->subscriber();
                                                                $articleUser = $article->user();
                                                            ?>
                                                            @if(
                                                            isset($authUser->where('user_id', $articleUser->select('id')->first()->id)->first()->id,
                                                            $articleUser->first()->subscription()->where('subscriber_id', auth()->user()->id)->first()->id)
                                                            &&
                                                            $authUser->where('user_id', $articleUser->select('id')->first()->id)->first()->id
                                                            ===
                                                            $articleUser->first()->subscription()->where('subscriber_id', auth()->user()->id)->first()->id)
